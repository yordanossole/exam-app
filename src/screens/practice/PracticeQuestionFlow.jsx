...
export default function PracticeQuestionFlow({ exam, questions, mode }) {
  ...
  return (
    <main style={shell}>
      ...
      <footer style={footer}>
        ...
        <Button variant="ghost" onClick={skip}>{isLast ? 'Finish' : 'Skip'}</Button>
        <Button disabled={submitting || !selectedLetter} onClick={next}>{isLast ? 'Submit' : 'Next'}</Button>
      </footer>
    </main>
  );
}
