...
export default function QuizScreen() {
  ...
  function doReveal(sel = selected) {
    ...
    setAnswers(prev => ({ ...prev, [question.id]: createAnswer(sel, true) }));
  }

  function createAnswer(sel, revealedAnswer) {
    return {
      option: sel,
      isCorrect: sel === question.answer.correct_option,
  ...
  function handleNext() {
    const nextAnswers = selected
      ? { ...answers, [question.id]: createAnswer(selected, revealed) }
      : answers;
    ...
    if (isLast) {
      navigate('/results/quiz-session', { state: { answers: nextAnswers, questions } });
    } else {
      jumpTo(index + 1);
    }
  }
  ...
  return (
    <div style={{ ...screenWrap, background: bgFlash, transition: 'background var(--duration-flash) ease' }}>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              {[
                { bg: 'var(--color-accent)',      label: 'Current' },
                { bg: 'var(--color-accent-tint)', label: 'Answered' },
                { bg: 'var(--color-track)',       label: 'Unanswered' },
              ].map(({ bg, label }) => (
      ...
      <main style={scrollContent}>
        ...
        <Button full size="lg" disabled={!selected || revealed} onClick={handleReveal}>
          {revealed ? 'Answer Shown' : 'Check Answer'}
        </Button>
      ...
      <footer style={footerBar}>
        ...
        <Button size="lg" disabled={!selected && !revealed} onClick={handleNext}>
