...
import { subjects, fetchExamsBySubject } from '../data/mockData';
...
export default function ExamListPage() {
  ...
  useEffect(() => {
    fetchExamsBySubject(subjectId).then(setExams);
  }, [subjectId]);
        ...
        <div style={{ background: '#ffffff', borderRadius: '0.75rem', overflow: 'hidden', border: '1px solid #e0e2ed' }}>
          {exams.map((exam, i) => (
            <div key={exam.id} style={{
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 500, color: '#181c23' }}>{exam.title}</div>
